/**
 * Tracking ligero: visitas y clics en clase de prueba.
 * - Contadores locales y GA4 solo con consentimiento de analítica (Ley 21.719).
 */

import { getCookieConsent, onCookieConsent } from './consent'

const STORAGE_KEY = 'isshin_temuco_analytics_v1'
...
export function exposeAnalyticsDebug() {
  ...
  ;(window as Window & { __isshinStats?: () => Counters }).__isshinStats = getAnalyticsSnapshot
}

/** Call once at boot: track when consent already given or when user accepts. */
